 ...
 for (num in 1:100) {
  ...
  for (gen in 1:896) {
   ...
   if ((gen > 396) && ((gen-396)%%10==0)) {
    write.table(t(table(pop.ceu$y)),paste(ofile.ceu,(896-gen),"genbp.txt"),row.names=FALSE,quote=FALSE,append=TRUE)
    write.table(t(table(pop.chb$y)),paste(ofile.chb,(896-gen),"genbp.txt"),row.names=FALSE,quote=FALSE,append=TRUE)
