...
sx.chb = r/0.038603061
sy.chb = sx.chb/1.84032582

sx.ceu = r/0.053629431
sy.ceu = sx.ceu/1.914662018


gname = paste("SULT1A3_r",r*1000,sep="")
...
fitlist.chb = rep(0,300)
fitlist.chb[1] = 1
fitlist.chb[2] = 1
for (y in 3:300) {
 t1 = (1+sx.chb)^(sum(k1^(0:(y-1))))
 t2 = (1-sy.chb)^(sum(k2^(0:(y-3))))
 fitlist.chb[y] = t1*t2
}

fitlist.ceu = rep(0,300)
fitlist.ceu[1] = 1
fitlist.ceu[2] = 1
for (y in 3:300) {
 t1 = (1+sx.ceu)^(sum(k1^(0:(y-1))))
 t2 = (1-sy.ceu)^(sum(k2^(0:(y-3))))
 fitlist.ceu[y] = t1*t2
}
...
ofile.ceu = paste("GADMAd_Output/",gname,"_GADMAd2_CEU_",sep="")
ofile.chb = paste("GADMAd_Output/",gname,"_GADMAd2_CHB_",sep="")
...
for (stp in 1:100) {
 ...
 for (num in 1:100) {
  ...
  pop.chb = pop.euas
  for (gen in 1:896) {
   pop.ceu = mhc.evolve.yonly(pop.ceu,fitlist.ceu,r,ne.ceu[gen])
   pop.chb = mhc.evolve.yonly(pop.chb,fitlist.chb,r,ne.chb[gen])
   ...
   if ((gen > 396) && ((gen-396)%%10==0)) {
    write.table(t(table(pop.ceu$y)),paste(ofile.ceu,(896-gen),"genbp.txt",sep=""),row.names=FALSE,quote=FALSE,append=TRUE)
    write.table(t(table(pop.chb$y)),paste(ofile.chb,(896-gen),"genbp.txt",sep=""),row.names=FALSE,quote=FALSE,append=TRUE)
